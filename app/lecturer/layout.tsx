...
const navItems = [
  { name: "Dashboard", href: "/lecturer/dashboard", icon: LayoutDashboardIcon },
  { name: "Assigned Courses", href: "/lecturer/courses", icon: BookOpenIcon },
  { name: "Live Session QR", href: "/lecturer/session", icon: QrCodeIcon },
  { name: "Reports & Exports", href: "/lecturer/reports", icon: FileSpreadsheetIcon },
];
        ...
        <div className="h-16 flex items-center justify-between px-6 border-b border-border">
          <Link href="/lecturer/dashboard" className="flex items-center gap-2">
            ...
            <span className="font-bold text-lg tracking-tight bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">
              QRAttend <span className="text-xs px-1.5 py-0.5 rounded bg-primary/10 text-primary uppercase font-mono">Lecturer</span>
            </span>
          </Link>
          <button
            onClick={() => setMobileMenuOpen(false)}
        ...
        <div className="p-4 border-t border-border flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 overflow-hidden">
            <Avatar className="size-9 border border-border">
              <AvatarImage src={user?.image || undefined} alt={user?.name || "Lecturer"} />
              <AvatarFallback className="bg-primary/10 text-primary font-semibold text-xs">
                {user?.name ? user.name.slice(0, 2).toUpperCase() : "LC"}
              </AvatarFallback>
            </Avatar>
            <div className="flex flex-col truncate">
              <span className="text-sm font-semibold truncate">{user?.name || "Lecturer"}</span>
      ...
      <div className="flex-1 flex flex-col lg:pl-64">
        ...
        <header className="h-16 border-b border-border bg-card/50 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between px-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-semibold text-lg hidden sm:block">Lecturer Attendance Dashboard</h1>
          </div>
